...
describe('Transport', () => {
  ...
  describe('createChannel(connecting, connected, callback)', () => {
    ...
    it('result should be an instance of Channel', () => {
      const createdChannel = port.createChannel(firstPeer, secondPeer, callback);
    ...
    it('should throw TypeError if connecting is not an isntance of PeerData', () => {
      const createChannelCaller = () => port.createChannel(invalidPeer, secondPeer, callback);
    ...
    it('should throw TypeError if connected is not an instance of PeerData', () => {
      const createChannelCaller = () => port.createChannel(firstPeer, invalidPeer, callback);

      expect(createChannelCaller).toThrow(/is not an instance of PeerData/);
    });
    it('should throw TypeError if getting peers are equal', () => {
      const createChannelCaller = () => port.createChannel(firstPeer, firstPeer, callback);

      expect(createChannelCaller).toThrow(/peers are equal/);
    });
    it('should throw TypeError if callback is not a function', () => {
      const createChannelCaller = () => port.createChannel(firstPeer, secondPeer, invalidCallback);

      expect(createChannelCaller).toThrow(/is not a function/);
    });
    it('should throw error if transport already has created channel', () => {
      port.createChannel(firstPeer, secondPeer, callback);

      const createChannelCaller = () => port.createChannel(firstPeer, secondPeer, callback);

      expect(createChannelCaller).toThrow(/already created/);
    });
    it('should catch error throwing by callback', () => {
    ...
    it('should throw error if returning by callback value hasn`t Channel-liked interface', () => {
